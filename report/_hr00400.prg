...
loSession = CREATEOBJECT("_hr00400")
...
ENDPROC
**********
DEFINE CLASS _hr00400 AS HotelSession OF ProcMultiProper.prg
...
SELECT histres.* FROM histres
     WHERE BETWEEN(hr_depdate, <<SqlCnvB(Min1)>>, <<SqlCnvB(Max1)>>)
ENDTEXT
...
Select  hr_ArrDate, hr_DepDate, hr_Rooms, hr_Adults, hr_Childs, hr_childs2, hr_childs3, 
	...
	a.ad_city, a.ad_member,
	rt_group, 0
From 	HistRes, RoomType, Address a, apartner, address b
Where 	Between(hr_depDate, Min1, Max1)
And 	hr_RoomTyp = Rt_RoomTyp 
And	Rt_Group <> 2 
And	a.Ad_AddrID = hr_AddrID
And	Iif(!empty(hr_compid), hr_compid, hr_addrid) = b.ad_addrid
And	Empty(hr_RoomNum)
And	Iif(empty(hr_apid), -9999, hr_apid) = ap_apid
order by 2
ENDTEXT
