...
PROCEDURE Init
     ...
     DODEFAULT("article,picklist,outoford,reservat,resrooms,resrate,houtofor,histres,hresroom,hresrate,param,roomtype,room,resrmshr,post,histpost,ressplit,address", tcHotCode, tcPath)
     IF this.lUseRemote
...
SELECT ooo.* FROM ( ;
     SELECT oo_fromdat, oo_todat, oo_roomnum FROM OutOfOrd ;
          WHERE (oo_fromdat <= pdLyEndDate AND oo_todat > pdLyStartDate OR oo_fromdat <= pdEndDate AND oo_todat > pdStartDate) AND NOT oo_cancel ;
     UNION ALL ;
     SELECT ho.oo_fromdat, ho.oo_todat, ho.oo_roomnum FROM HOutOfOr ho ;
     LEFT JOIN OutOfOrd ON OutOfOrd.oo_id = ho.oo_id ;
          WHERE (ho.oo_fromdat <= pdLyEndDate AND ho.oo_todat > pdLyStartDate OR ho.oo_fromdat <= pdEndDate AND ho.oo_todat > pdStartDate) AND NOT ho.oo_cancel AND ISNULL(OutOfOrd.oo_id) ;
     ) ooo ;
...
LOCAL i, lnAmount, lcPpField, lcBuilding
...
SCAN
     ...
     FOR i = 1 TO 2
          ...
          IF NOT EMPTY(min3) AND ar_sub = min3
...
DO WHILE ldDate <= MIN(pdEndDate, curReservat.rs_depdate)
     ...
     IF (EVL(RptBulding,"*") = "*" OR lcBuildng = RptBulding) AND NOT ((llSharing OR EMPTY(lnRooms)) AND EMPTY(lnAdults) AND EMPTY(lnChilds1) AND EMPTY(lnChilds2) AND EMPTY(lnChilds3))
          ...
          FOR i = 1 TO 2
               ...
               FOR k = 1 TO 2
                    ...
                    FOR l = 1 TO 3
                         DO CASE
                              CASE l = 1 AND ldDate <= MIN(IIF(i=1,pdEndDate,pdLyEndDate), MAX(curReservat.rs_arrdate,curReservat.rs_depdate-1))
...
PROCEDURE PpDo
LPARAMETERS tlWithoutVat, tcResFilter, tcAdrFilter
LOCAL lcHResFilter, lcResFilterC, lcHResFilterC, lcAdrFilterC, lcVatnumMacro, lcArchScripts, lcPostDate, lcHistPostDate
...
pdStartDate = min1
pdEndDate = max1
pdLyStartDate = GetRelDate(min1,"-1Y")
pdLyEndDate = GetRelDate(max1,"-1Y")
IF VARTYPE(RptUseBookKeepingDate)="X"
...
SELECT histres.* FROM histres
     ...
     SELECT hr_reserid AS c_reserid FROM histres
          WHERE hr_reserid >= 1 AND (hr_arrdate <= <<SqlCnvB(pdLyEndDate)>> AND hr_depdate >= <<SqlCnvB(pdLyStartDate)>> OR hr_arrdate <= <<SqlCnvB(pdEndDate)>> AND hr_depdate >= <<SqlCnvB(pdStartDate)>>)
     UNION
     SELECT hr_reserid FROM histres
          INNER JOIN histpost ON hp_reserid = hr_reserid OR hp_origid = hr_reserid
          WHERE hp_reserid > 0 AND (BETWEEN(hp_date, <<SqlCnvB(pdLyStartDate)>>, <<SqlCnvB(pdLyEndDate)>>) OR BETWEEN(hp_date, <<SqlCnvB(pdStartDate)>>, <<SqlCnvB(pdEndDate)>>))
          GROUP BY hr_reserid
...
SELECT histpost.* FROM histpost
     WHERE hp_reserid > 0 AND (BETWEEN(hp_date, <<SqlCnvB(pdLyStartDate)>>, <<SqlCnvB(pdLyEndDate)>>) OR BETWEEN(hp_date, <<SqlCnvB(pdStartDate)>>, <<SqlCnvB(pdEndDate)>>))
ENDTEXT
ProcArchive("RestoreArchive", "histres,histpost", lcArchScripts, pdLyStartDate)
*
****************************************************************************************************
...
lcPostDate = IIF(RptUseBookKeepingDate,"ps_date","EVL(ps_rdate,ps_date)")
lcHistPostDate = IIF(RptUseBookKeepingDate,"hp_date","EVL(hp_rdate,hp_date)")
SELECT NVL(ar_main,0) AS ar_main, NVL(ar_sub,0) AS ar_sub, &lcPostDate AS ps_date, ps_amount, ps_vat0, ps_vat1, ps_vat2, ps_vat3, ps_vat4, ps_vat5, ps_vat6, ps_vat7, ps_vat8, ps_vat9, ;
     ...
     WHERE ps_reserid > 0 AND (BETWEEN(&lcPostDate, pdLyStartDate, pdLyEndDate) OR BETWEEN(&lcPostDate, pdStartDate, pdEndDate)) AND ;
...
UNION ALL ;
SELECT NVL(ar_main,0), NVL(ar_sub,0), &lcHistPostDate AS hp_date, hp_amount, hp_vat0, hp_vat1, hp_vat2, hp_vat3, hp_vat4, hp_vat5, hp_vat6, hp_vat7, hp_vat8, hp_vat9, ;
     ...
     WHERE hp_reserid > 0 AND (BETWEEN(&lcHistPostDate, pdLyStartDate, pdLyEndDate) OR BETWEEN(&lcHistPostDate, pdStartDate, pdEndDate)) AND ISNULL(ps_postid) AND ;
