...
loSession = CREATEOBJECT("_bn00104")
...
ENDPROC
**********
DEFINE CLASS _bn00104 AS HotelSession OF ProcMultiProper.prg
...
SELECT histres.* FROM histres
     ...
     SELECT bn_reserid FROM billnum
          WHERE BETWEEN(bn_cxldate, <<SqlCnvB(min1)>>, <<SqlCnvB(max1)>>)
          GROUP BY bn_reserid
...
where between(bn_cxldate, min1, max1)
