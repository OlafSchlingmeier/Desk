...
CREATE CURSOR PreProc (pp_year N(4), pp_month N(2), pp_code C(10), pp_maxrms N(8), pp_rev B(2), ;
     ...
     pp_ohg6 B(2), pp_ohg7 B(2), pp_ohg8 B(2), pp_ohg9 B(2), pp_ohg0 B(2))
INDEX ON PADL(pp_year,4,'0')+PADL(pp_month,2,'0')+pp_code TAG pp_code
ENDPROC
*
...
DO WHILE DATE(lnYear, lnMonth, 1) <= max1
     ...
     SELECT SUM(MIN(oo_todat-1,ldEndDay)-MAX(oo_fromdat,ldStartDay)+1) FROM ( ;
          ...
          LEFT JOIN OutOfOrd ON OutOfOrd.oo_id = ho.oo_id ;
               WHERE ho.oo_fromdat <= ldEndDay AND ho.oo_todat > ldStartDay AND NOT ho.oo_cancel AND ISNULL(OutOfOrd.oo_id) ;
          ) ooo ;
...
SELECT curPost
SCAN FOR SEEK(LEFT(DTOC(curPost.ps_date,1),6)+curPost.rescode, "PreProc", "pp_code") OR SEEK(LEFT(DTOC(curPost.ps_date,1),6)+[*****], "PreProc", "pp_code")
...
DO WHILE ldDate <= MIN(max1, curReservat.rs_depdate)
     ...
     IF NOT (llSharing OR EMPTY(lnRooms) AND EMPTY(lnRsAdults) AND EMPTY(lnRsChilds1) AND EMPTY(lnRsChilds2) AND EMPTY(lnRsChilds3)) AND ;
               SEEK(LEFT(DTOC(ldDate,1),6)+lcCode, "PreProc", "pp_code") OR SEEK(LEFT(DTOC(ldDate,1),6)+[*****], "PreProc", "pp_code")
...
LOCAL lcHResFilter, lcCodeField, lcCodeHField, lcCodeSField, lcVatnumMacro, lcArchScripts
