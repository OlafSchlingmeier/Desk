...
PARAMETER m.fname, m.cpbyte, m.noclosefiles
IF SET("TALK") = "ON"
   SET TALK OFF
   m.mtalk = "ON"
ELSE
...
DIMENSION error_array[c_maxerror]
...
PROCEDURE cz_errormsg
PARAMETER num
WAIT WINDOW error_array[num] NOWAIT
*
PROCEDURE setcodepagetotables
LOCAL l_nCodePage, l_cTable
l_nCodePage = INT(VAL(INPUTBOX("Enter code page","Input","1252")))
IF EMPTY(l_nCodePage) OR NOT openfiledirect(.F.,"files")
     RETURN .F.
ENDIF
DO SetMessagesOff IN procmessages
allowlogin(.f.)

SELECT * FROM files INTO CURSOR curfiles READWRITE
CloseFile("files")
SELECT curfiles
SCAN FOR "DATA" $ fi_path AND UPPER(ALLTRIM(fi_name)) == UPPER(ALLTRIM(fi_alias))
     l_cTable = FORCEEXT(ALLTRIM(fi_name),"dbf")
     WAIT WINDOW "Table: " + l_cTable NOWAIT
     cpzero(l_cTable,l_nCodePage)
     SELECT curfiles
ENDSCAN
CloseFile("curfiles")

SetReportTablesCodePage(l_nCodePage)
...
ENDPROC
*
PROCEDURE SetReportTablesCodePage
LPARAMETERS lp_nCodePage
* Set code page for report dbf lang tables
LOCAL p_cPath
LOCAL l_cFileSkeleton, l_nFound, i, l_cFileName, l_cDBF, l_cTableName
p_cPath = FULLPATH("report")
l_cFileSkeleton = ADDBS(p_cPath)+"*.dbf"
l_nFound = ADIR(l_aDir,l_cFileSkeleton)
IF l_nFound = 0
     RETURN
ENDIF

FOR i = 1 TO ALEN(l_aDir,1)
     l_cFileName = LOWER(l_aDir(i,1))
     l_cTableName = FORCEEXT(l_cFileName,"")
     l_cDBF = ADDBS(p_cPath)+FORCEEXT(l_cFileName,"dbf")
     IF FILE(l_cDBF)
          WAIT WINDOW "Table: " + l_cTableName NOWAIT
          cpzero(l_cDBF,lp_nCodePage)
     ENDIF
ENDFOR
ENDPROC
*